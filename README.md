...
| [运行技能锁](catalog/runtime-skill-lock.json) | 检查自研源、用户级安装副本和外部 UI/UX 依赖状态 |
| [Task Council Skill](skills/task-council/SKILL.md) | 为复杂任务按需组建角色会议，保留分歧并产出可验证的决策与行动项 |
| [Product Design Management Skill](skills/product-design-management/SKILL.md) | 需求分析、产品设计、产品规范、PRD、路线图和 PM 工作流 |
| [Oristrat Product UI Governor](skills/oristrat-product-ui-governor/SKILL.md) | 产品 UI 设计、审计、改版、参考设计研究、视觉质量和浏览器验收门禁 |
| [Fullstack Delivery Orchestration Skill](skills/fullstack-delivery-orchestration/SKILL.md) | PM 主控、全栈开发、QA 点击测试、多 agent 长任务交付编排 |
| [MSCE Engine App Development Skill](skills/msce-engine-app-development/SKILL.md) | MSCE 开发、组件边界、staged scope 和基线感知门禁 |
...
| Skill | 路径 | 定位 |
|---|---|---|
| `task-council` | [skills/task-council/SKILL.md](skills/task-council/SKILL.md) | 通用任务智囊会：按议题选择职责角色，并为同一角色配置倡议、审慎、落地等不同席位，形成证据、异议、决策与行动闭环 |
...
| `oristrat-product-ui-governor` | [skills/oristrat-product-ui-governor/SKILL.md](skills/oristrat-product-ui-governor/SKILL.md) | 业务适配的产品 UI 总控，统一界面设计、审计、改版、Hallmark 外部桥接和最终质量证据 |
| `fullstack-delivery-orchestration` | [skills/fullstack-delivery-orchestration/SKILL.md](skills/fullstack-delivery-orchestration/SKILL.md) | PM 主控的全栈交付编排，协调全栈工程师、QA 点击测试、评审、发布门禁和长任务循环 |
| `msce-engine-app-development` | [skills/msce-engine-app-development/SKILL.md](skills/msce-engine-app-development/SKILL.md) | MSCE / MortiseSpecCodeEngine 开发、审查、staged scope、独立日志和基线归因 |
...
9. 修改正式 Skill 后运行 `tools/validate_skills_utf8.ps1` 和 `tools/check_skill_install_sync.ps1`；同步用户级安装副本必须单独授权。
